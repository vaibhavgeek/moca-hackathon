...
const CredentialVerification = () => {
  ...
  const [config, setConfig] = useState({
    verifierAuth: "your-verifier-auth-token",
    programId: "program-123",
    projectName: "AIR Credential Demo",
    projectLogo: "https://via.placeholder.com/150x50/3B82F6/FFFFFF?text=AIR+Demo",
    redirectUrlForIssuer: "https://example.com/issue-credential",
  });
      ...
      <div className="max-w-6xl mx-auto bg-white rounded-lg shadow-lg p-6 sm:p-8">
        ...
        {verificationResult && (
          <div className="mb-6 p-6 border rounded-md">
            <div className={`p-4 rounded-md border ${getStatusColor(verificationResult.status)}`}>
              <div className="flex items-center mb-2">
                <span className="text-2xl mr-3">{getStatusIcon(verificationResult.status)}</span>
                <h4 className="text-lg font-semibold">Verification Result</h4>
              </div>
              <p className="text-sm mb-2">
                <strong>Status:</strong> {verificationResult.status}
              </p>
              <p className="text-sm">{getStatusDescription(verificationResult.status)}</p>
            </div>
          </div>
        )}
        ...
        <div className="mt-8 p-4 bg-blue-50 border border-blue-200 rounded-md">
          ...
          <ul className="text-sm text-blue-800 space-y-1">
            <li>• Configure your verifier auth token and program ID</li>
            <li>• Set the redirect URL for users who need to obtain credentials</li>
            <li>• Click "Start Credential Verification" to start the process</li>
            <li>• The widget will guide the user through the credential verification flow</li>
            <li>• You'll see the verification result with detailed status information</li>
          </ul>
        </div>

        {/* Status Reference */}
        <div className="mt-6 p-4 bg-gray-50 border border-gray-200 rounded-md">
          <h4 className="text-sm font-medium text-gray-900 mb-3">Verification Status Reference:</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs">
            <div className="flex items-center">
              <span className="mr-2">✅</span>
              <span className="text-green-700">Compliant - Credential is valid</span>
            </div>
            <div className="flex items-center">
              <span className="mr-2">❌</span>
              <span className="text-red-700">Non-Compliant - Does not meet requirements</span>
            </div>
            <div className="flex items-center">
              <span className="mr-2">⏳</span>
              <span className="text-yellow-700">Pending - Waiting for blockchain confirmation</span>
            </div>
            <div className="flex items-center">
              <span className="mr-2">🔄</span>
              <span className="text-orange-700">Revoking - Credential is being revoked</span>
            </div>
            <div className="flex items-center">
              <span className="mr-2">🚫</span>
              <span className="text-orange-700">Revoked - Credential has been revoked</span>
            </div>
            <div className="flex items-center">
              <span className="mr-2">⏰</span>
              <span className="text-gray-700">Expired - Credential has expired</span>
            </div>
            <div className="flex items-center">
              <span className="mr-2">🔍</span>
              <span className="text-purple-700">NotFound - No credential found</span>
            </div>
          </div>
        </div>
      </div>
